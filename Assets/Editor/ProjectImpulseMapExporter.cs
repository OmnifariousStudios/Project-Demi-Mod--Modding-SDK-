...
public class ProjectImpulseMapExporter : EditorWindow {
    ...
    void LoadConfiguredGamemodes() 
    {
        ...
        string[] tags = new string[1]; //profile.editableModProfile.tags.value;
    ...
    private void OnGUI() 
    {
        ...
        if (profile == null) {
            ...
            EditorGUILayout.HelpBox(profile + " will be updated as used as the upload target on the server.", MessageType.Info);
        } else {
            EditorGUILayout.HelpBox(profile + " will be created as a new profile on the server.", MessageType.Info);
        }
        profile = EditorGUILayout.ObjectField("Mod Profile", profile, typeof(GameObject), false) as GameObject;
        if (profile)
        ...
        if (profile) {
            containsMapTag = profile.name.Contains("Map");
        }
        if (profile == null)
        ...
        using (new EditorGUI.DisabledScope(true || platformValidation != "" || !containsMapTag)) 
        {
...
*/
    ...
    private void ModProfileSubmissionSucceeded(ModProfile updatedProfile, string profileFilePath) 
    {
